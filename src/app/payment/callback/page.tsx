...
import { clearActiveBooking, formatFlightPrice, getBookingDetails } from '@/services/whitelabel-api';

import type { BookingDetails } from '@/types/whitelabel';

function PaymentCallbackContent() {
	const searchParams = useSearchParams();
	const reference = searchParams.get('reference') ?? searchParams.get('trxref') ?? searchParams.get('booking_ref');
	...
	const isSuccess = status === 'success' || status === 'successful' || searchParams.get('success') === 'true';
		...
		return (
			<Container className="py-20">
				<div className="glossy-card mx-auto max-w-md p-8 text-center">
					...
					<p className="mt-2 text-muted-foreground">Your flight has been booked. A confirmation email has been sent.</p>
					<div className="mt-6 space-y-3 rounded-lg bg-muted p-4 text-left text-sm">
						...
						{bookingDetails.amount != null && (
							<div>
								...
								<p>{formatFlightPrice(Number(bookingDetails.amount), bookingDetails.currency ?? 'NGN')}</p>
							</div>
						)}
					</div>
					<Link
		...
		return (
			<Container className="py-20">
				<div className="glossy-card mx-auto max-w-md p-8 text-center">
					...
					<p className="mt-2 text-muted-foreground">Your payment was successful, but we couldn&apos;t load booking details right now.</p>
					{reference && (
	...
	return (
		<Container className="py-20">
			<div className="glossy-card mx-auto max-w-md p-8 text-center">
				...
				<p className="mt-2 text-muted-foreground">Your payment could not be processed. Please try again.</p>
				{reference && (
					<div className="mt-6 rounded-lg bg-muted p-4 text-left">
						<p className="text-sm font-medium">Reference</p>
						<p className="font-mono text-sm">{reference}</p>
