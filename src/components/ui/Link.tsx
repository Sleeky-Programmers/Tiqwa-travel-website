...
const linkVariants = cva("outline-none transition-all duration-200 focus-visible:ring-3 focus-visible:ring-ring/50 rounded-sm", {
  variants: {
    variant: {
      ...
      sidebar: "group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-foreground/60 hover:bg-primary/5 hover:text-foreground hover:translate-x-0.5",
      /** Dropdown / overflow menu item — icon + label row inside a popover or menu panel. */
      "menu-item": "flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-foreground/70 transition-colors hover:bg-primary/5 hover:text-foreground",
  ...
  compoundVariants: [
    {
      variant: "nav",
      active: true,
      className: "bg-primary text-white shadow-sm shadow-primary/30 hover:bg-primary hover:text-white",
    },
    {
      variant: "sidebar",
      active: true,
...
function Link({ className, variant = "default", active = false, children, ...props }: LinkProps) {
  if (variant === "footer") {
    return (
      <NextLink data-slot="link" className={cn(linkVariants({ variant, active, className }))} {...props}>
  ...
  if (variant === "back") {
    return (
      <NextLink data-slot="link" className={cn(linkVariants({ variant, active, className }), "hover:gap-3")} {...props}>
