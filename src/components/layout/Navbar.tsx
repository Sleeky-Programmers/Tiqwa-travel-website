	...
	return (
		<>
			...
			<motion.header
				...
				{/* Main nav */}
				<div className={`bg-background-card border-b border-border transition-shadow duration-300${scrolled ? ' shadow-[0_4px_20px_-4px_rgba(0,0,0,0.1)]' : ''}`}>
					<nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
						{/* Logo */}
						<Link
							href="/"
							className="flex items-center gap-2 group">
							<BrandLogo className="text-base text-foreground transition-colors group-hover:text-primary md:text-lg" />
						</Link>
										...
										<motion.div
											...
											<Link
												...
												className={`flex items-center justify-between rounded-xl px-4 py-3 text-base font-medium transition-all duration-200 ${
													isActive ? 'bg-primary text-white' : 'text-foreground/70 hover:bg-primary/8 hover:text-foreground'
												}`}>
												{link.label}
