...
import { Calendar, Gift, LayoutDashboard, LogOut, Search, User } from 'lucide-react';
									...
									return (
										<Link
											...
											className={cn(
												...
												active ? 'dashboard-nav-link-active' : 'text-foreground/65 hover:bg-primary/5 hover:text-foreground'
											)}>
											<item.icon className={cn('h-4 w-4 shrink-0 transition-colors duration-200', active ? 'text-primary' : 'text-muted-foreground')} />
											{item.label}
										</Link>
									);
								})}
