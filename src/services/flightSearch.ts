import { FlightRoute, popularRoutes } from '@/data/mockData';
import { parseDuration } from '@/services/mockFlights';
...
function routeToFlights(route: FlightRoute): Flight[] {
	return routeSchedules.map((schedule, i) => ({
		...
		airlineLogo: route.image ? route.image : null,
		from: route.from,
		to: route.to,
...
export function searchFlights(from: string, to: string, filters: FlightFilters = {}): Flight[] {
	const fromRoutes = popularRoutes.filter((r) => matchesRoute(r, from, to)).flatMap(routeToFlights);

	const merged = new Map<string, Flight>();
	[...fromRoutes].forEach((flight) => {
